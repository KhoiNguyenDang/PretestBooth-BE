...
export const CreateBoothSchema = z.object({
  ...
  code: z
    .string()
    .trim()
    .min(3, 'Mã booth tối thiểu 3 ký tự')
    .max(50, 'Mã booth tối đa 50 ký tự')
    .optional(),
...
export const UpdateBoothSchema = z
  .object({
    name: z.string().min(1).optional(),
    code: z
      .union([
        z.string().trim().min(3, 'Mã booth tối thiểu 3 ký tự').max(50, 'Mã booth tối đa 50 ký tự'),
        z.null(),
      ])
      .optional(),
    description: z.union([z.string(), z.null()]).optional(),
    location: z.union([z.string(), z.null()]).optional(),
    status: z.enum(['ACTIVE', 'MAINTENANCE_PENDING', 'MAINTENANCE', 'INACTIVE']).optional(),
    statusNote: z
      .string()
      .trim()
      .min(3, 'Ghi chú trạng thái tối thiểu 3 ký tự')
      .max(300)
      .optional(),
  })
...
export const TransferBoothBookingsSchema = z.object({
  targetBoothId: z.string().uuid('ID booth đích không hợp lệ'),
  reason: z.string().trim().min(3, 'Lý do tối thiểu 3 ký tự').max(300),
  dryRun: z.boolean().optional().default(false),
  includeCheckedIn: z.boolean().optional().default(true),
  bookingIds: z.array(z.string().uuid('ID booking không hợp lệ')).min(1).optional(),
});

export type TransferBoothBookingsDto = z.output<typeof TransferBoothBookingsSchema>;
